...
def expand_console(page) -> None:
    page.wait_for_selector(".hardcopy-console-ui", timeout=10000)
    console = page.locator(".hardcopy-console-ui").first
    if console.evaluate("el => el.classList.contains('is-minimized')"):
        page.locator("[data-hardcopy-header]").first.click()
...
def main() -> int:
    print("hardcopy-console: regenerate docs/media")
    started: list[subprocess.Popen] = []
    try:
        started = start_apps()
        paths = capture_apps()
