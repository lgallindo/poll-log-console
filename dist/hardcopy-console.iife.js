...
(function (global) {
  ...
  var FRAGMENT = '<div class="hardcopy-console-ui is-minimized" data-hardcopy-root>' +
    '<div class="hardcopy-header" data-hardcopy-header><div class="hardcopy-header-left">' +
    '<div class="notif-light" data-hardcopy-notif></div>' +
    '<span data-hardcopy-title-full style="display:none"></span><span data-hardcopy-title-mini>C:\\</span></div>' +
    '<div class="hardcopy-header-actions" data-hardcopy-actions style="display:none">' +
    '<button type="button" data-hardcopy-copy title="Copy Logs">📋</button>' +
    '<button type="button" data-hardcopy-clear title="Clear Logs">🗑️</button><span>[-]</span></div></div>' +
    '<div class="hardcopy-body" data-hardcopy-body hidden><div data-hardcopy-lines></div>' +
    '<div class="hardcopy-prompt"><span class="hardcopy-prompt-gt">&gt;</span>' +
    '<input type="text" data-hardcopy-input autocomplete="off"><div class="hardcopy-cursor"></div></div></div></div>';
  ...
  function mount(target, opts) {
    ...
    if (!el) throw new Error('HardcopyConsole.mount: target not found');
    var store = createStore(opts);
    var root = el.querySelector('[data-hardcopy-root]');
    if (!root) { el.innerHTML = FRAGMENT; root = el.querySelector('[data-hardcopy-root]'); }
    var notif = root.querySelector('[data-hardcopy-notif]');
    var titleFull = root.querySelector('[data-hardcopy-title-full]');
    var titleMini = root.querySelector('[data-hardcopy-title-mini]');
    var actions = root.querySelector('[data-hardcopy-actions]');
    var body = root.querySelector('[data-hardcopy-body]');
    var lines = root.querySelector('[data-hardcopy-lines]');
    var input = root.querySelector('[data-hardcopy-input]');
    ...
    function render() {
      ...
      lines.innerHTML = store.state.logs.map(function (log) {
        return '<div class="hardcopy-line"><span class="hardcopy-line-time">' + escapeHtml(log.time || '') +
          '</span><span class="hardcopy-msg-' + (log.type || 'sys') + '">' + escapeHtml(log.msg || '') + '</span></div>';
      }).join('');
    ...
    root.querySelector('[data-hardcopy-header]').addEventListener('click', function (e) {
      e.stopPropagation();
    ...
    root.querySelector('[data-hardcopy-copy]').addEventListener('click', function (e) {
      e.stopPropagation(); handleCommand(store, notify, ':w'); render();
    });
    root.querySelector('[data-hardcopy-clear]').addEventListener('click', function (e) {
  ...
  function bindHtmxBeep(bodySelector, options) {
    ...
    bodySelector = bodySelector || '#hardcopy-body';
  ...
  var api = {
    ...
    registerAlpine: function (Alpine, name) {
      Alpine.data(name || 'hardcopyConsole', alpineComponent);
    },
    sound: { beep: playNotifSound }
  };
  global.HardcopyConsole = api;
